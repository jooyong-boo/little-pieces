...
살아 있는 것들을 다루는 법(주소·배포·확인·계정)은 [handoff.md](handoff.md)에 있다.
...
배포가 끝나고 실제로 써보며 나온 요구다. **전부 완료.**

|                                                   |     |
| ------------------------------------------------- | --- |
| 목록에 사진이 한 장만 보임 → 우측 상단 장수 배지  | ✅  |
| 날짜를 손으로 `YYYY-MM-DD` 타이핑 → 네이티브 피커 | ✅  |
| 지도에서 그 자리에 바로 등록                      | ✅  |
| 지도에서 현재 위치로 이동                         | ✅  |
| 커플의 "처음 만난 날"도 피커로 (선택 필드)        | ✅  |

**`@expo/ui`가 이미 설치돼 있었다** — iOS Pod(`Podfile.lock`의 `ExpoUI`)까지 빌드에 들어가
있는데 `src/`에서 한 번도 안 쓰이고 있었다. 날짜 피커에 새 의존성이 필요 없었다.
반대로 **`expo-location`은 새로 넣어야 했다** — `showsUserLocation`만으로 iOS가 권한을
물어볼 거라 본 게 실기기에서 틀렸다.

**지도 등록은 길게 누르기가 아니라 버튼이 주 경로다.** 원래는 "접근 가능한 경로가 이미
있으니 지도 전용 버튼은 만들지 않는다"였는데, 그 판단은 길게 누르기가 동작한다는 전제
위에 있었다. 실기기에서는 손가락이 조금만 움직여도 지도 팬으로 넘어가 잘 안 잡힌다.
전제가 깨진 만큼 결론도 바꿔 "여기에 남기기"를 두고, 길게 누르기는 지름길로 남겼다.

> **실기기에서만 드러난 것이 셋이었다.** 위치 권한 프롬프트가 안 뜬 것, 버튼이 탭바에
> 완전히 가려진 것(시뮬레이터에서는 아슬아슬하게 보였다), 지도를 한 번도 안 움직이면
> 중앙 좌표가 (0, 0)으로 들어간 것. Maestro는 항상 팬을 거쳐서 마지막 것을 못 잡았다.
> **지도와 권한은 시뮬레이터로 검증했다고 말할 수 없다.**
