...
- **`quality: 0.7`을 줬는데도 HEIC 원본이 2.7MiB 그대로 올라갔다**(관찰된 사실).
  왜 안 줄었는지는 확인하지 않았다 — 다만 서버측 리사이즈가 없는 지금은
  큰 파일이 큰 채로 저장된다는 뜻이고, 사진이 쌓이면 이게 비용의 대부분이 된다.
- HEIC는 iOS에서만 안전하게 렌더된다. Android/웹까지 가면 변환이 필요하다.
- **MinIO에 고아 객체가 쌓이고 있다** — E2E와 실구동에서 만든 것들 중 어떤 추억에도
  붙지 않은 게 여럿이다. "고아 객체 정리"를 미룬 결과가 이미 눈에 보이는 형태로 나타났다.
- Maestro 사진 플로우는 **콜드 부팅 후에도 통과**한다(웜 상태 3회 + 콜드 2회).
