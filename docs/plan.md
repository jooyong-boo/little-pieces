...
3. **`fitToCoordinates`는 마운트 직후엔 무시된다** — 지도가 아직 네이티브 레이아웃 전이라서다.
   API 자체는 멀쩡하다(버튼으로 호출하면 정상 동작하는 것을 확인). 그래서 첫 화면은
   `regionForCoordinates()`로 계산해 `initialRegion`에 넣고(타이밍 무관), 추억이 늘었을 때의
   재조정만 `fitToCoordinates`에 맡긴다.
