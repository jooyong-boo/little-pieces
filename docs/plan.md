...
## 진행 상황 (2026-08-27) — 3차(지도) 완료

- [x] 0단계 경로 정리, 1·2차(사진 업로드) 완료
- [x] **3차 지도** — 위치 선택 Modal, 지도 탭(마커 + 방문일 순 경로선), 폼 연동.
      iOS 시뮬레이터에서 전 구간 실구동 확인. Maestro `memory-with-location.yaml` 통과.
- [ ] **Android 지도** — `GOOGLE_MAPS_ANDROID_KEY`가 아직 없다. 키가 생기면
      `.env`에 넣고 빌드 → SHA-1 추출 → 애플리케이션 제한 순서로 진행.

### 실구동에서 확인한 것 (지도)

1. **iOS가 Apple Maps로 동작한다** — `Podfile.lock`에 GoogleMaps 언급이 0개.
   플러그인 소스에서 읽은 `useGoogleMaps: !!iosGoogleMapsApiKey` 조건이 실제로 그렇게 동작한다.
2. **중앙 고정 핀을 이모지에서 원형 도트로 바꿨다.** 이모지 📍는 글리프 안에서
   뾰족한 끝 위치가 폰트마다 달라, 사용자가 보는 지점과 저장되는 좌표가 어긋난다.
   원은 중심이 곧 지점이라 어긋날 수 없다.
3. **`fitToCoordinates`가 마운트 직후 호출에서는 효과가 없었다**(관찰된 사실).
   같은 API를 버튼으로 나중에 부르면 정상 동작한다 — 즉 API가 깨진 게 아니라
   호출 시점의 문제다. **정확한 이유는 확인하지 않았다**(지도의 네이티브 레이아웃 전이라는
   가설이 유력하지만 검증하지 않음). 그래서 첫 화면은 `regionForCoordinates()`로 계산해
   `initialRegion`에 넣어 타이밍에 아예 의존하지 않게 하고, 추억이 늘었을 때의 재조정만
   `fitToCoordinates`에 맡긴다. 이 구조는 원인을 몰라도 안전하다.
4. 위치 제거 → 저장이 지도까지 반영되고, 좌표가 하나도 없으면 빈 상태 안내가 뜬다.

### 지도에서 미룬 것

- **역지오코딩** — 핀을 찍으면 `placeName`이 자동으로 채워지면 좋지만 Geocoding API 요금이 붙는다.
- **마커 클러스터링** — 추억이 수백 개가 되기 전엔 무의미하다.
- **현재 위치** — `expo-location` + 위치 권한. 지도에서 찍는 것으로 충분하다.
- **Android 탭 아이콘** — drawable 리소스 미비. 1차부터 동일한 상태.
- 개발 DB에 좌표가 있는 테스트 추억 3건이 남아 있다(명동/사당/종로 부근). 무해하지만
  나중에 "이게 뭐지" 하지 않도록 적어둔다.
